...
namespace FastQuant
{
    ...
    public class QuickLZ
    {
        ...
        public static int qlz_get_setting(int setting)
        {
            throw new NotSupportedException();
        }
                    ...
                    if ((cword_val & 1) == 1)
                    {
                        ...
                        if (level == 1)
                        {
                            ...
                            while (last_hashed < dst - matchlen)
                            {
                                ...
                                hash_counter[hash] = 1;
                                fetch = (uint)((uint)(fetch >> 8 & 0xffff) | (uint)(destination[last_hashed + 3] << 16));
                            }
                            fetch = (uint)(source[src] | (source[src + 1] << 8) | (source[src + 2] << 16));
                        }
                        else
                    ...
                    else
                    {
                        if (dst <= last_matchstart)
                        {
                            ...
                            if (level == 1)
                            {
                                ...
                                fetch = (uint)((uint)(fetch >> 8 & 0xffff) | (uint)(source[src + 2] << 16));
                            }
                            else
                            {
                                fetch = (uint)((uint)(fetch >> 8 & 0xffff) | (uint)(source[src + 2] << 16) | (uint)(source[src + 3] << 24));
                            }
                        }
                        else
                        {
