...
namespace FastQuant
{
    ...
    {
        public ControllerUser(string username, string password)
        {
            UserName = username;
            Password = password;
        }

        public bool IsDefault() => Equals(this, default(ControllerUser));

        public bool Equals(ControllerUser other) => UserName == other.UserName && Password == other.Password;

        public override bool Equals(object obj) => obj is ControllerUser && Equals((ControllerUser)obj);

        public override int GetHashCode()
        {
            var r = UserName?.GetHashCode() ^ Password?.GetHashCode();
            return !r.HasValue ? 0 : r.GetValueOrDefault();
        }

        public override string ToString() => !IsDefault() ? $"{UserName} / {Password}" : "All";
